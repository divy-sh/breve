...
const { modelStatus, getModelStatus } = useModels();
const { getConfig, setConfig } = useSettings();
...
onMounted(async () => {
    ...
    unlisten = await listen<{ id: string; content: string }>(
        "llm-stream",
        (event) => {
    ...
    loadConversation(await getConfig("lastConversationId")).catch(
        console.error,
    );
});
