...
<template>
    <k-list-item>
        ...
        <template #subtitle>
            ...
            <div v-if="isExpanded">
                ...
                <k-chip class="m-0.5 max-w-full">
                    <span class="truncate"
                        >Size: {{ model["size" as any] }} MB</span
                    >
                </k-chip>
            ...
            <template v-if="downloadedModels.includes(props.modelName)">
                ...
                <k-button
                    ...
                    inline
                    @click="onSetDefault()"
                    :disabled="isDefault(props.modelName)"
                    class="w-1/3"
                >
                    <i
                        v-if="isSettingDefault === props.modelName"
                        class="pi pi-spinner pi-spin"
                    ></i>
                    <i
                        v-else
                        :class="
                            isDefault(props.modelName)
                                ? 'pi pi-check-circle'
                                : 'pi pi-play-circle'
                        "
                    >
                ...
                <k-button
                    clear
                    inline
                    @click="confirmDelete(props.modelName)"
                    :disabled="isDefault(props.modelName)"
                    class="w-1/3"
                >
            ...
            <k-button
                ...
                class="w-1/3"
            >
                <i :class="isExpanded ? 'pi pi-angle-up' : 'pi pi-angle-down'">
                    Details</i
                >
            </k-button>
        </template>
    ...
    <k-dialog
        ...
        @backdrop-click="showDeleteConfirm = false"
    >
        <p class="mb-4">
        ...
        <div class="flex gap-2">
            <k-button @click="showDeleteConfirm = false" outline
                >Cancel</k-button
            >
            <k-button @click="proceedDelete" class="k-color-brand-red"
                >Delete</k-button
            >
        </div>
    </k-dialog>
